<script lang="ts">
    ...
    const availableModels = [
        "gemini-2.0-flash",
        "gemini-1.5-flash-latest", // Often a good default
        "gemini-1.5-pro-latest",
        "gemini-1.0-pro", // Older, might have different pricing/capabilities
    ...
    async function sendAiRequest() {
      ...
      try {
        ...
        // 6. Call the Edge Function
         const functionUrl = `${PUBLIC_SUPABASE_URL}/functions/v1/gemini-ai-stream`;
                ...
                while (boundary !== -1) {
                    ...
                    if (line.startsWith('data: ')) {
                        ...
                        try {
                            ...
                            // console.log("Parsed Gemini Chunk:", chunk); // <<< Log to see structure

                            // --- !!! PARSE GEMINI STRUCTURE !!! ---
                            // Gemini typically sends text in: chunk.candidates[0].content.parts[0].text
                            const delta = chunk?.candidates?.[0]?.content?.parts?.[0]?.text;
                            // ---!!!----------------------------!!! ---

                            if (typeof delta === 'string' && delta.length > 0) {
                                updateLastAssistantMessage(delta, $store.currentModel);
                                requestAnimationFrame(() => { /* scroll */ if (responseElement) responseElement.scrollTop = responseElement.scrollHeight; });
                            } else {
                                // Check for finish reason if available
                                const finishReason = chunk?.candidates?.[0]?.finishReason;
                                if (finishReason && finishReason !== 'STOP') { // Log reasons other than normal stop
                                    console.log("Chunk finishReason:", finishReason);
                                }
                            }
                        } catch (e) {
                            // Ignore simple parsing errors if expecting JSON that isn't there
                            // Only log persistent errors or malformed data if needed
                            if (!data.includes('[DONE]')) { // Avoid logging the common DONE marker if present
                                console.warn("Couldn't parse stream JSON chunk:", data, e);
                            }
                        }
                    }
